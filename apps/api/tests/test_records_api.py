...
async def _record(
    ...
    *,
    ...
    async with session_scope(assume_app_role=False) as session:
        user_id = (
            await session.execute(sa.select(User.id).order_by(User.created_at.asc()).limit(1))
        ).scalar_one()
        session.add(
            PersonalRecord(
                user_id=user_id,
...
class TestBestPerType:
    async def test_only_the_best_row_survives(self, client: AsyncClient) -> None:
        """Aynı türde üç rekor satırı var; listede bir tanesi görünmeli."""
        exercise, _ = await _two_exercises()
        await _record(exercise, "max_weight", "70", reps=8, day=0)
        await _record(exercise, "max_weight", "77.5", reps=8, day=7)
        await _record(exercise, "max_weight", "80", reps=8, day=14)
    ...
    async def test_more_reps_wins_at_equal_weight(self, client: AsyncClient) -> None:
        """"100kg x 1" ile "100kg x 8" aynı rekor değil."""
        exercise, _ = await _two_exercises()
        await _record(exercise, "max_weight", "100", reps=1, day=0)
        await _record(exercise, "max_weight", "100", reps=8, day=7)

        rows = (await client.get("/workouts/records/best")).json()
        weight = next(r for r in rows[0]["records"] if r["type"] == "max_weight")
        assert weight["reps"] == 8

    async def test_each_type_kept_separately(self, client: AsyncClient) -> None:
        exercise, _ = await _two_exercises()
        await _record(exercise, "max_weight", "80", reps=8)
        await _record(exercise, "max_reps", "12")
        await _record(exercise, "estimated_1rm", "96")
    ...
    async def test_types_come_in_a_stable_order(self, client: AsyncClient) -> None:
        ...
        await _record(exercise, "session_volume", "9000", day=0)
        await _record(exercise, "max_reps", "12", day=1)
        await _record(exercise, "estimated_1rm", "96", day=2)
        await _record(exercise, "max_weight", "80", reps=8, day=3)
    ...
    async def test_the_date_is_when_it_was_set_not_when_matched(
        ...
        exercise, _ = await _two_exercises()
        await _record(exercise, "max_weight", "80", reps=8, day=0)
        await _record(exercise, "max_weight", "80", reps=8, day=30)
...
class TestGroupingAndOrder:
    async def test_grouped_by_exercise_with_its_name(self, client: AsyncClient) -> None:
        first, second = await _two_exercises()
        await _record(first, "max_weight", "80", reps=8)
        await _record(second, "max_weight", "40", reps=12)
    ...
    async def test_freshest_achievement_comes_first(self, client: AsyncClient) -> None:
        """Kullanıcı en son neyi kırdığını listenin başında görmeli."""
        first, second = await _two_exercises()
        await _record(first, "max_weight", "80", reps=8, day=0)
        await _record(second, "max_weight", "40", reps=12, day=20)
    ...
    async def test_last_achieved_is_the_newest_in_the_group(
        ...
        exercise, _ = await _two_exercises()
        await _record(exercise, "max_weight", "80", reps=8, day=0)
        await _record(exercise, "max_reps", "12", day=40)

        rows = (await client.get("/workouts/records/best")).json()
        assert rows[0]["last_achieved_at"].startswith("2026-07-11")  # 1 Haziran + 40

    async def test_no_records_returns_empty(self, client: AsyncClient) -> None:
